 ...
 * @author Laptop88
 */
public class ChooseLessonViewNext extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (lid.equals("")) {
            ...
            for (User_Lesson listlestatu : listlearn) {
                if (listlestatu.getLesson().getId() > lessonid && count < 1) {
                    lesson = ld.getLessonById(listlestatu.getLesson().getId());
                    count++;
                }
            }
            if (lesson == null) {
                lesson = ld.getLessonById(listlearn.get(listlearn.size()).getLesson().getId());
            }
        }
